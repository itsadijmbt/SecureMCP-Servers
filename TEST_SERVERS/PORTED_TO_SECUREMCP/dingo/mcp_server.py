...
# from fastmcp import FastMCP    # PORT: was the third-party fastmcp package; dingo only uses
                                  #       FastMCP("name") + @mcp.tool() decorators, both of which
                                  #       SecureMCP supports identically. Drop-in swap.
...
# PORT: was `mcp = FastMCP("Dingo Evaluator")`. SecureMCP is the
# MACAW-instrumented replacement; the 6 @mcp.tool() decorators below
# are unchanged.
mcp = SecureMCP("Dingo Evaluator")
...
# PORT: original branched on LOCAL_DEPLOYMENT_MODE to pick stdio
# (mcp.run()) vs SSE (mcp.run(transport="sse")). Under SecureMCP the
# `transport=` arg is inert -- traffic arrives via the MACAW mesh, not
# stdio or SSE. Both branches collapse to a plain mcp.run(). The
# LOCAL_DEPLOYMENT_MODE branch is kept as a no-op switch for callers
# whose deploy scripts still set the env var; the body is identical.
#
...
if __name__ == "__main__":
    # Both deployment modes converge under SecureMCP -- the mesh
    # carries traffic regardless of the LOCAL_DEPLOYMENT_MODE env var.
    mcp.run()
