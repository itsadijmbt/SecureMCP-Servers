...
# import nest_asyncio                                    # PORT: not needed; SecureMCP owns its single asyncio.run()
from dotenv import load_dotenv
# from mcp.server.fastmcp import FastMCP, Context        # PORT
# from mcp.types import ToolAnnotations                  # PORT: SecureMCP @mcp.tool() does not accept annotations=
from macaw_adapters.mcp import SecureMCP, Context
from mcp.shared.exceptions import McpError                # PORT: KEPT -- exception class only, used at line 685
...
# mcp = FastMCP("telegram")    # PORT
mcp = SecureMCP("telegram")
...
async def _get_effective_allowed_roots_with_status(
    ...
    # ==================================================================
    # PORT NOTE -- Path A active (server-side roots only).
    # See MIGRATION.txt for the full A vs B trade-off.
    # ------------------------------------------------------------------
    # Original used `await ctx.session.list_roots()` -- the MCP-protocol
    # capability for the CLIENT to declare its filesystem roots at
    # session init. SecureMCP has no equivalent protocol slot
    # (mcp.py:38-470: no Context.session.list_roots; only a sync
    # ctx.get_roots() which returns SERVER-declared roots from CLI args).
    #
    # We chose Path A: the server's --allowed-root CLI args are the
    # only source of authorization. The ctx argument is kept in the
    # signature for API stability but not consulted for roots; we
    # document this loss as BROKEN ON PURPOSE in MIGRATION.txt.
    #
    # Path B (preserved below as commented alternative): clients call
    # a new `register_roots` tool once per session and roots get
    # stashed in MACAW session state. Path B preserves the original
    # per-session client-supplied semantic but adds a client-side
    # onboarding step. We rejected B because telegram-mcp deployments
    # are typically single-tenant with admin-set roots; Path A is the
    # smaller, faithful-to-architecture port for that shape.
    # ==================================================================
    # OLD (FastMCP path -- removed):
    # try:
    ...
    #
    # PATH B ALTERNATIVE (preserved for future maintainers, not active):
    # try:
    #     stored = await ctx.get_state("client_roots")
    # except Exception:
    #     logger.error(
    #         "Failed to read client_roots from session state; "
    #         "falling back to SERVER_ALLOWED_ROOTS.",
    #         exc_info=True,
    #     )
    #     stored = None
    # if not stored:
    ...
    # for raw in stored:
    #     try:
    #         client_roots.append(_coerce_root_uri_to_path(str(raw)))
    #     except Exception:
    #         continue
    # if client_roots:
    #     return _dedupe_paths(client_roots), ROOTS_STATUS_READY
    ...
    if fallback_roots:
        # ctx is accepted for API stability but ignored for roots
        # under Path A. Server-side roots win for every caller.
...
# ======================================================================
# PORT: PATH B ALTERNATIVE (NOT ACTIVE).
# ----------------------------------------------------------------------
# Preserved as a commented reference for future maintainers. We chose
# Path A (server-side CLI roots only). Path B would expose a tool that
# lets a client declare its allowed filesystem roots per session, with
# storage in MACAW session state via ctx.set_state("client_roots", ...).
#
# Why we did NOT pick Path B:
#   - Telegram-mcp deployments are typically single-tenant with admin-
#     set roots; the dynamic-client-roots feature was inherited from
#     MCP-protocol completeness, not from a specific telegram product
#     requirement.
#   - Path B adds a client-side onboarding step (call register_roots
#     before any file tool works). Path A requires no client change.
#   - Skill rule "do not change architecture" strictly favours A.
#
# When you would want to switch to Path B:
#   - Multi-tenant deployment where each client has a different roots
#     set and the admin cannot enumerate them at startup.
#   - You have client(s) that historically relied on the FastMCP
#     session.list_roots() flow and you want behavioural parity.
#
# To activate Path B: uncomment this tool body AND uncomment the
# Path B branch inside `_get_effective_allowed_roots_with_status`.
# ======================================================================
# @mcp.tool()
# async def register_roots(ctx: Context, roots: List[str]) -> dict:
#     """Declare the caller's allowed filesystem roots for this session.
#
#     Call this once at session start before invoking any file-path tool
#     (upload_file, download_media, ...) if you need the server to honour
#     a client-supplied root list. If you skip this call, file tools fall
#     back to whatever roots the server admin set via the
#     --allowed-root CLI argument (SERVER_ALLOWED_ROOTS).
#
#     Args:
#         roots: A list of absolute filesystem paths or file:// URIs to
#                declare as allowed roots for this session.
#
#     Returns:
#         A dict with the count of roots accepted (after dedup/validation)
#         and the count rejected.
#     """
#     accepted: List[str] = []
#     rejected: List[str] = []
#     for raw in roots or []:
#         try:
#             _coerce_root_uri_to_path(str(raw))
#             accepted.append(str(raw))
#         except Exception:
#             rejected.append(str(raw))
#     await ctx.set_state("client_roots", accepted, serializable=False)
#     return {
#         "accepted": len(accepted),
#         "rejected": len(rejected),
#         "rejected_entries": rejected,
#         "message": (
#             f"Registered {len(accepted)} root(s) for this session"
#             + (f"; {len(rejected)} rejected" if rejected else "")
#         ),
#     }


# @mcp.tool(annotations=ToolAnnotations(title="List Accounts", readOnlyHint=True))    # PORT: stripped annotations=
@mcp.tool()
...
@mcp.tool()    # PORT: stripped annotations=
...
# ======================================================================
# PORT NOTE -- entry point rewrite
# ----------------------------------------------------------------------
# Original used an async _main() that:
#   1. eagerly called `cl.start()` on every Telegram client (anyio
#      scope opens here, bound to the loop);
#   2. eagerly called `cl.get_dialogs()` to warm entity caches;
#   3. awaited `mcp.run_stdio_async()` on the SAME loop;
#   4. on shutdown, awaited `cl.disconnect()` for every client.
#
# Under SecureMCP, mcp.run() is sync and creates its own asyncio.run()
# internally (mcp.py:628-635). Calling cl.start() at module-level
# would bind the Telegram session's anyio scope to a loop that
# closes before SecureMCP starts -- the dead-reactor pattern we
# documented in the skill (Step 4).
#
# Replacement strategy:
#   * `cl.start()` is dropped; `ensure_connected(cl)` (line ~231)
#     already does lazy connect-on-first-use inside tool handlers,
#     where the await happens on SecureMCP's running loop. Same
#     functional outcome, slightly slower first call.
#   * `cl.get_dialogs()` cache warming is dropped. It was a pure
#     optimisation; first lookup that needs a dialog will pay one
#     extra Telegram round-trip, then telethon caches.
#   * `cl.disconnect()` cleanup is dropped. SecureMCP exposes no
#     async shutdown hook. OS reclaims sockets at process exit;
#     Telegram-side this is benign (sessions stay valid, the
#     connection just times out server-side).
#   * `nest_asyncio.apply()` is dropped. There is no nested loop
#     under SecureMCP -- mcp.run() owns the only asyncio.run().
#   * Replace `asyncio.run(_main())` with a plain sync main() that
#     calls mcp.run().
#
...
def main() -> None:
    ...
    labels = ", ".join(clients.keys())
    print(
        f"Configured {len(clients)} Telegram account(s) ({labels}). "
        "Connections open lazily on first tool call.",
        file=sys.stderr,
    )
    try:
        mcp.run()
