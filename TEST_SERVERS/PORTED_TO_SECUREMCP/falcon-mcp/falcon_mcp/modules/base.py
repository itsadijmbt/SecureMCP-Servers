...
# from falcon_mcp.macaw import secure_tool
# this was for shim feature i,e http -> macaw_mesh
...
class BaseModule(ABC):
    ...
    @abstractmethod
    def register_tools(self, server: SecureMCP) -> None:
    ...
    def register_resources(self, server: SecureMCP) -> None:
    ...
    def _add_tool(
        ...
        #    add_tool() is a FastMCP method. It registers a callable with FastMCP's internal routing table. SecureMCP doesn't have add_tool() — it has .tool(), which is a decorator that stores the
#   function and its metadata in self._tools. Later, when .run() calls _init_macaw_client(), those stored tools become MACAWClient handlers.

        server.tool( name=prefixed_name)(method)
        self.tools.append(prefixed_name)
        logger.debug("Added tool: %s", prefixed_name)

    def _add_resource(self, server: SecureMCP, resource: Resource) -> None:
        ...
        """
        # 
        # # SecureMCP expects its own Resource type, cast accordingly
        ...
        # since macaw registred it in tool with func we need to wrap it in a closure
        uri  = str(resource.uri)
        text = resource.text
        @server.resource(uri,description=resource.description)
        def handler() ->str:
            return text
        self.resources.append(uri)
        logger.debug("Added resource: %s", uri)
