...
# from falcon_mcp.macaw import macaw_auth_middleware
...
class FalconMCPServer:
    ...
    def __init__(
        ...
        # stateless_http: bool = False, no http
        client_id: str | None = None,
        client_secret: str | None = None,
        # api_key: str | None = None,  # no static-key auth
        ...
        logger.warning("Skipping Falcon API auth (dev mode)")
        ...
        #     port=self.port,
        # )
        self.server  = SecureMCP("securemcp-falcon-mcp")
    ...
    def _register_tools(self) -> int:
        ...
        self.server.tool(name="falcon_check_connectivity")(self.falcon_check_connectivity)
        self.server.tool(name="falcon_list_enabled_modules")(self.list_enabled_modules)
        self.server.tool( name="falcon_list_modules")(self.list_modules)
...
# REMOVING THE MIDDLEWARE CAUSE MACAW IS NOW IN B/W
    ...
    def run(self, 
            # transport: TransportType = "stdio" <- no choice req
             ) -> None:
        # OLD
        # """Run the MCP server.
        ...
        # NEW : routed via macaw
        logger.info("Starting falcon-mcp on MACAW mesh")
        self.server.run()
        # natively this uses run_aysnc + init_client
...
def main() -> None:
    ...
    try:
        ...
        logger.info("Starting server on MACAW mesh")
        server.run()
