...
from macaw_adapters.mcp import Context, SecureMCP
...
class QdrantMCPServer(SecureMCP):
    """
    ...
    def __init__(
        ...
        name: str = "securemcp-server-qdrant",
        # instructions: str | None = None,
        
        **kwargs,
    ):
        # super().__init__(name="securemcp-qdrant", **kwargs)
    ...
    def setup_tools(self):
        ...
        async def store(
            ...
            """
            ctx.debug(f"Storing information {information} in Qdrant")
        ...
        async def find(
            ...
            # Log query_filter
            ctx.debug(f"Query filter: {query_filter}")

            query_filter = models.Filter(**query_filter) if query_filter else None

            ctx.debug(f"Finding results for query {query}")
