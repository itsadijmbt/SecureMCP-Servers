...
# from mcp.server.fastmcp import FastMCP                # PORT
...
# PORT: Initialize SecureMCP server. Original constructor (kept commented):
#   mcp = FastMCP(
#       "excel-mcp",
...
#   )
# Notes:
#   - host=, port=, instructions= are not accepted by SecureMCP
#     (skill Step 1b). Dropped. Under SecureMCP the mesh carries
#     traffic; HTTP host/port and the MCP-protocol instructions
#     string are protocol-layer concerns that no longer apply.
#   - FASTMCP_HOST / FASTMCP_PORT env vars become unused under
#     this port. Kept the env var names visible in the comment
#     above for anyone migrating an existing deployment.
mcp = SecureMCP("excel-mcp")
...
# ======================================================================
# PORT NOTE -- transport functions
# ----------------------------------------------------------------------
# WHAT changed: the original three functions called
#   mcp.run(transport="sse")
#   mcp.run(transport="streamable-http")
#   mcp.run(transport="stdio")
# Under SecureMCP `mcp.run()` ignores the transport= kwarg and creates
# its own asyncio.run() internally (mcp.py:628-635). Traffic arrives
# via the MACAW mesh, not over stdio/SSE/streamable-http. So all three
# functions now call `mcp.run()` plain.
#
# WHY we keep three functions: __main__.py exposes them as three typer
# subcommands (`sse`, `streamable_http`, `stdio`). Existing
# `python -m excel_mcp <transport>` invocations should continue to
# work without rewriting; under the port they all dispatch to the same
# SecureMCP boot, but the user-facing CLI shape is unchanged. Skill
# rule "least modification" -- keep the function shape, neutralize
# only what must be neutralized.
#
# The EXCEL_FILES_PATH directory creation and the `EXCEL_FILES_PATH`
# global assignment are kept verbatim in run_sse and run_streamable_http
# because they are business logic (the server uses this directory to
# resolve relative file paths in tool calls), not transport machinery.
# ======================================================================

def run_sse():
    """Run Excel MCP server (originally SSE; now SecureMCP / mesh under the port)."""
    ...
    try:
        logger.info(f"Starting Excel MCP server (SecureMCP) (files directory: {EXCEL_FILES_PATH})")
        # mcp.run(transport="sse")    # PORT: was SSE; transport arg is inert under SecureMCP.
        mcp.run()
...
def run_streamable_http():
    """Run Excel MCP server (originally streamable HTTP; now SecureMCP / mesh under the port)."""
    ...
    try:
        logger.info(f"Starting Excel MCP server (SecureMCP) (files directory: {EXCEL_FILES_PATH})")
        # mcp.run(transport="streamable-http")    # PORT: was streamable-http; inert under SecureMCP.
        mcp.run()
    except KeyboardInterrupt:
        logger.info("Server stopped by user")
    except Exception as e:
...
def run_stdio():
    """Run Excel MCP server (originally stdio; now SecureMCP / mesh under the port)."""
    # No need to assign EXCEL_FILES_PATH in stdio mode (kept comment from original).

    try:
        logger.info("Starting Excel MCP server (SecureMCP)")
        # mcp.run(transport="stdio")    # PORT: was stdio; inert under SecureMCP.
        mcp.run()
    except KeyboardInterrupt:
        logger.info("Server stopped by user")
    except Exception as e:
