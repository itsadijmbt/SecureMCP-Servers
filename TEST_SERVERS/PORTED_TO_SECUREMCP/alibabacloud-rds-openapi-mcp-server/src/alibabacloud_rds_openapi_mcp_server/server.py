...
# PORT: anyio + uvicorn + starlette imports were used only for the deleted
# HTTP/SSE transport branch and VerifyHeaderMiddleware. Commented out so
# the file doesn't carry unused dependencies (and so installs without
# uvicorn/starlette work).
# import anyio
# import uvicorn
...
# PORT: ToolAnnotations is still imported because READ_ONLY_TOOL is referenced
# in @mcp.tool(annotations=READ_ONLY_TOOL) decorations. The decorator's data
# collector accepts it, but activate() filters it out before passing to
# SecureMCP's .tool() (which doesn't accept annotations). Kept for the
# READ_ONLY_TOOL constant; could be removed if you also drop that constant.
...
from utils import metadata_as_request_headers   # add to existing utils import
...
# PORT: dropped `port=...` and `stateless_http=True`. Both were FastMCP-specific
# transport tuning options. SecureMCP serves over the MACAW mesh and accepts
# neither — leaving them in would be silently absorbed via **kwargs into the
# inner SecureMCP's extra_config, then forwarded to MACAWClient where they may
# raise depending on which keyword args MACAWClient accepts. Cleanest is to
# omit them entirely.
mcp = RdsMCP("Alibaba Cloud RDS OPENAPI")
try:
...
# now the anontations dont impact as the in .activate annoation as arg are not processed 
@mcp.tool(annotations=READ_ONLY_TOOL)
async def describe_db_instances(ctx:Context,region_id: str):
    """
    Queries instances.
    Args:
...
@mcp.tool(annotations=READ_ONLY_TOOL)
async def describe_db_instance_attribute(ctx:Context,region_id: str, db_instance_id: str):
    """
...
@mcp.tool(annotations=READ_ONLY_TOOL)
async def describe_db_instance_performance(ctx:Context,
                                           region_id: str,
                                           db_instance_id: str,
                                           db_type: str,
                                           perf_keys: list[str],
...
@mcp.tool()
async def modify_parameter(ctx: Context, 
        region_id: str,
        dbinstance_id: str,
...
@mcp.tool()
async def modify_db_instance_spec(ctx: Context, 
        region_id: str,
        dbinstance_id: str,
...
@mcp.tool(annotations=READ_ONLY_TOOL)
async def describe_available_classes(ctx: Context, 
        region_id: str,
        zone_id: str,
...
@mcp.tool()
async def create_db_instance(ctx: Context, 
        region_id: str,
        engine: str,
        engine_version: str,
...
@mcp.tool(annotations=READ_ONLY_TOOL)
async def describe_available_zones(ctx: Context, 
        region_id: str,
        engine: str,
...
@mcp.tool(annotations=READ_ONLY_TOOL)
async def describe_vpcs(ctx: Context, 
        region_id: str,
        vpc_id: str = None,
        vpc_name: str = None,
        resource_group_id: str = None,
...
@mcp.tool(annotations=READ_ONLY_TOOL)
async def describe_vswitches(ctx: Context, 
        region_id: str = None,
        vpc_id: str = None,
        vswitch_id: str = None,
        zone_id: str = None,
...
@mcp.tool(annotations=READ_ONLY_TOOL)
async def describe_slow_log_records(ctx: Context, 
        region_id: str,
        dbinstance_id: str,
        start_time: str,
        end_time: str,
...
@mcp.tool(annotations=READ_ONLY_TOOL)
async def describe_error_logs(ctx: Context, 
        region_id: str,
        db_instance_id: str,
        start_time: str,
        end_time: str,
...
@mcp.tool(annotations=READ_ONLY_TOOL)
async def describe_db_instance_net_info(ctx: Context, 
        region_id: str,
...
@mcp.tool(annotations=READ_ONLY_TOOL)
async def describe_db_instance_ip_allowlist(ctx: Context, 
        region_id: str,
        db_instance_ids: list[str]
) -> list[dict]:
    """
...
@mcp.tool(annotations=READ_ONLY_TOOL)
async def describe_db_instance_databases(ctx: Context, 
        region_id: str,
        db_instance_ids: list[str]
) -> list[dict]:
    """
...
@mcp.tool(annotations=READ_ONLY_TOOL)
async def describe_db_instance_accounts(ctx: Context, 
        region_id: str,
        db_instance_ids: list[str]
) -> list[dict]:
    """
...
@mcp.tool()
async def create_db_instance_account(ctx: Context, 
        region_id: str,
        db_instance_id: str,
        account_name: str,
...
@mcp.tool(annotations=READ_ONLY_TOOL)
async def describe_db_instance_parameters(ctx: Context, 
        region_id: str,
        db_instance_ids: list[str],
...
@mcp.tool(annotations=READ_ONLY_TOOL)
async def describe_bills(ctx: Context, 
...
@mcp.tool()
async def modify_db_instance_description(ctx: Context, 
        region_id: str,
        db_instance_id: str,
        description: str
):
...
@mcp.tool()
async def allocate_instance_public_connection(ctx: Context, 
        region_id: str,
        db_instance_id: str,
...
@mcp.tool(annotations=READ_ONLY_TOOL)
async def describe_all_whitelist_template(ctx: Context, 
        region_id: str,
...
@mcp.tool(annotations=READ_ONLY_TOOL)
async def describe_instance_linked_whitelist_template(ctx: Context, 
        region_id: str,
        db_instance_id: str
):
    """
...
@mcp.tool()
async def attach_whitelist_template_to_instance(ctx: Context, 
        region_id: str,
        db_instance_id: str,
        template_id: int
):
...
@mcp.tool()
async def add_tags_to_db_instance(ctx: Context, 
        region_id: str,
        db_instance_id: str,
        tags: Dict[str, str]
):
...
@mcp.tool()
async def modify_security_ips(ctx: Context, 
        region_id: str,
        dbinstance_id: str,
        security_ips: str,
...
@mcp.tool()
async def restart_db_instance(ctx: Context, 
        region_id: str,
        dbinstance_id: str,
...
@mcp.tool(annotations=READ_ONLY_TOOL)
async def describe_monitor_metrics(ctx: Context, 
        dbinstance_id: str,
...
@mcp.tool(annotations=READ_ONLY_TOOL)
async def describe_sql_insight_statistic(ctx: Context, 
        dbinstance_id: str,
        start_time: str,
        end_time: str,
) -> Dict[str, Any]:
...
@mcp.tool(annotations=READ_ONLY_TOOL)
async def show_engine_innodb_status(ctx: Context, 
        dbinstance_id: str,
        region_id: str
) -> str:
    """
...
@mcp.tool(annotations=READ_ONLY_TOOL)
async def show_create_table(ctx: Context, 
        region_id: str,
        dbinstance_id: str,
        db_name: str,
        table_name: str
...
@mcp.tool(annotations=READ_ONLY_TOOL)
async def explain_sql(ctx: Context, 
        region_id: str,
        dbinstance_id: str,
        db_name: str,
        sql: str
...
@mcp.tool(annotations=READ_ONLY_TOOL)
async def query_sql(ctx: Context, 
        region_id: str,
        dbinstance_id: str,
        db_name: str,
        sql: str
...
async def show_largest_table(ctx: Context, 
...
async def show_largest_table_fragment(ctx: Context, 
        region_id: str = Field(description="The region ID of the RDS instance."),
        dbinstance_id: str = Field(description="The ID of the RDS instance."),
        topK: int = Field(default=5, description="To display the number of the top few largest tables")
) -> str:
...
# PORT: health_check endpoint removed. It was an HTTP-transport-only
# probe (Starlette Request/JSONResponse) for container/load-balancer
# health checks. Under SecureMCP/stdio there is no HTTP server to
# expose it on, so the function is unreachable dead code.


# PORT: VerifyHeaderMiddleware is deleted. Under SecureMCP no ASGI app
# is constructed (no sse_app / streamable_http_app), so there's nothing
# to attach middleware to. Two responsibilities the middleware had:
#   1. API-key validation (Authorization Bearer check)  → replaced by
#      MACAW signed invocations validated by the Local Agent BEFORE
#      the handler runs. Stronger guarantee than bearer comparison.
#   2. ->(this is imp)"Populating" `current_request_headers` from HTTP headers  → replaced by `metadata_as_request_headers(ctx)` 
# in utils.py, called from each
#      tool body. Same ContextVar, different source.
#
...
#         return response
# this is the upsteam auth handled by utility<->contextvar<->helper
# 
# 
# 
...
def main(toolsets: Optional[str] = None) -> None:
    ...
    # PORT: HTTP/SSE transport branch deleted. SecureMCP has no
    # sse_app()/streamable_http_app(), doesn't accept ASGI middleware,
    # and traffic rides the MACAW mesh — not HTTP. Caller-side API-key
    # auth (VerifyHeaderMiddleware) is replaced by MACAW signed
    # invocations.
    #
    # Old code (kept as a banner for the next reader):
